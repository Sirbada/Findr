...
export function Card({ className = '', variant = 'default', hoverable = false, ...props }: CardProps) {
  const base = 'rounded-xl border border-[#E8E8E4] overflow-hidden'
  const variants = {
  ...
  const hover = hoverable ? 'transition-all duration-200 hover:shadow-[0_4px_16px_rgba(0,0,0,0.06),0_1px_4px_rgba(0,0,0,0.04)] hover:border-[#D4D4CE] hover:-translate-y-0.5 cursor-pointer' : ''
